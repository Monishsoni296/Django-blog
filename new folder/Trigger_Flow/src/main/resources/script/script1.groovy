...
def Message processData(Message message) {
    ...
    def headers = message.getHeaders();
    def query = headers.get("CamelHttpQuery");
    message.setHeader("oldHeader", query + " modified");
    
    if (query != null && query.contains("address=")) {

        def address = query.substring(query.indexOf("address=") + 8)

        // Handle multiple params if present
        if (address.contains("&")) {
        ...
        message.setHeader("Address", address)
    }
